...
MapDevice()
{
    ...
    /* Map the Spice Command buffer */
    si->ramArea = map_physical_memory("Spice Command buffer",
        (addr_t)si->ramDma, si->ramSize, B_ANY_KERNEL_BLOCK_ADDRESS|B_MTR_WC,
        B_READ_AREA|B_WRITE_AREA, (void **)&si->ram);
    if (si->ramArea < 0) {
		/* Try again without write combining */
		writeCombined = 0;
        si->fbArea = map_physical_memory("Spice Command buffer",
            (addr_t)si->ramDma, si->ramSize, B_ANY_KERNEL_BLOCK_ADDRESS,
            B_READ_AREA|B_WRITE_AREA, (void **)&si->ram);
	}
    if (si->ramArea < 0) {
        TRACE("failed to map command buffer\n");
        return si->ramArea;
	}
    TRACE("command buffer mapped: %p->%p, area %" B_PRId32 ", size %" B_PRId32 ", write "
        "combined: %" B_PRId32 "\n", si->ramDma, si->fb, si->ramArea,
        si->ramSize, writeCombined);

    /* Map the Spice Surface buffer */
    si->vramArea = map_physical_memory("Spice Surface buffer",
        (addr_t)si->vramDma, si->vramSize, B_ANY_KERNEL_BLOCK_ADDRESS,
        B_READ_AREA|B_WRITE_AREA, (void **)&si->vram);
    if (si->vramArea < 0) {
        TRACE("failed to map surface buffer\n");
        delete_area(si->vramArea);
        return si->vramArea;
	}

    /* Map the Spice Parameter buffer */
    si->romArea = map_physical_memory("Spice Parameter buffer",
        (addr_t)si->romDma, si->romSize, B_ANY_KERNEL_BLOCK_ADDRESS,
        B_READ_AREA|B_WRITE_AREA, (void **)&si->rom);
    if (si->romArea < 0) {
        TRACE("failed to map parameter buffer\n");
        delete_area(si->romArea);
        return si->romArea;
    }
